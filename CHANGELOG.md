...
- Generic two-sided query filters and live label resolution.
- Single ChangePlan mutation boundary with field-level read-back verification.
- Fail-closed rendering for inconsistent relation results.
- Post-write verification before reporting success.

### Changed
- All read operations remain silent and bypass the write Gatekeeper.
- Mutation dependencies are resolved from live NetBox results; no business identifiers are embedded.
- UI responses use structured business rendering instead of raw MCP JSON.

### Security
- User feedback remains review telemetry and is never injected into LLM instructions.
- Per-tab session isolation and stale-plan protection remain enforced.
...
- Document the required `dcim.device` fields and management-IP creation order in the agent prompt.


### Added
- Single-approval ReAct continuation after the first confirmed write result.
- Quick-reply metadata for clarification questions.
- Server-side history deduplication for repeated assistant messages.


### Added
- Contextual cross-referencing for ambiguous terms using user context and NetBox observations.
- Natural clarification questions when identity remains uncertain.
- Generic no-hardcoding reasoning rules for aliases and manufacturer/model resolution.


### Added
- Natural-language Pending plans with bullet points instead of raw MCP JSON/function calls.
- Mandatory broad alias searches before Manufacturer or Device Type creation.


### Added
- Full RW plan summary before confirmation.
- Explicit prerequisite-autonomy instructions for complete infrastructure requests.
- Versioned Gatekeeper system prompt for ordered multi-tool execution.
...
### Fixed
- Lock consultation keywords and follow-up clarification replies to the read-only route.
- Support global device listing after `liste des devices` → `tous les sites`.
- Prevent read-only requests from reaching the mutation planner.

## [0.6.3] - 2026-08-05

### Fixed
- Route explicit device-list queries through the read-only site/device gateway path.
- Resolve display names such as `FR01 - Le Fief-Sauvin` without entering mutation planning.
- Add detailed exception logging around chat view failures.

## [0.6.2] - 2026-08-05

### Fixed
- Route greetings and explicit site-list requests through the read-only gateway without creating a PendingPlan.
- Return sanitized JSON errors from the chat endpoint instead of raw HTML 500 responses.


### Fixed
- Enforce resolver-issued provenance certificates at the v0.6 pipeline boundary.
- Reject forged or unobserved `ResolvedRef` instances before planning.
- Validate exact read-only backend matches and preserve distinct multi-template collections during count expansion.
...
### Changed
- Adopt strict SemVer patch releases; the current runtime, package metadata, UI version, and release tag are now `0.5.2`.
- Future fixes increment the patch component directly (`0.5.3`, `0.5.4`, etc.).
