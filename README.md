...
```bash
python -m build
pip install dist/netwaive-0.3.12-py3-none-any.whl
```
...
```bash
/opt/netbox/venv/bin/pip install dist/netwaive-0.3.12-py3-none-any.whl
/opt/netbox/venv/bin/python /opt/netbox/netbox/manage.py check
systemctl restart netbox netbox-rq
```

La version `0.3.12` inclut le widget global flottant/docké et une page de chat dédiée. Le widget est injecté via `PluginTemplateExtension` lorsque le plugin est activé dans `PLUGINS`.
