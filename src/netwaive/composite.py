...
from .models import NDX_COMPONENT_ENDPOINTS, NDXImportPayload, ToolResult
...
class NDXCompositeImporter:
    ...
    def run(self, raw_payload: dict[str, Any]) -> ToolResult:
        ...
        device_type = payload.device_type.model_dump(exclude={"front_image", "rear_image"}, exclude_none=True)
        device_type["manufacturer"] = manufacturer_id
        parent = self._create_or_reuse("device-types", device_type)
        if not parent.ok:
            return parent
        parent_id = parent.data.get("id") if isinstance(parent.data, dict) else None
        if not isinstance(parent_id, int) or parent_id <= 0:
            return ToolResult(ok=False, message="Import NDX interrompu : le DeviceType n’a retourné aucun identifiant.")

        processed = 0
        for collection, endpoint in self.ENDPOINTS.items():
            for component in payload.component_templates.get(collection, []):
                result = self._create_or_reuse(endpoint, {**component, "device_type": parent_id})
                if not result.ok:
                    return result
                processed += 1
        return ToolResult(ok=True, message=f"Import NDX terminé : {processed} templates traités.", data={"id": parent_id, "templates_processed": processed})
